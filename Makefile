.PHONY: help up down restart build rebuild logs ps shell migrate makemigrations \
...
build:  ## Собрать образы (без перезапуска контейнеров)
	docker compose build

rebuild:  ## Пересобрать образы и перезапустить контейнеры (+nginx, чтобы освежить upstream DNS)
...
shell:  ## Открыть sh внутри web-контейнера
