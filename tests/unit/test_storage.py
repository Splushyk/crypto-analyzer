"""
Модуль unit-тестирования компонентов хранилища.

Основные проверки:
1. Валидация логики формирования JSON-отчета: структура словаря, ключи и форматирование даты.
2. Изоляция файловой системы: использование фикстуры mocker (pytest-mock) для
   перехвата операций записи (I/O) без создания реальных файлов.
3. Проверка корректности передачи аргументов в системные вызовы (open, json.dump).

Все тесты в этом модуле являются быстрыми и выполняются полностью в памяти.
"""
