...
* **Models**: `Snapshot`, `CoinPrice`, `WatchlistItem`, `Portfolio`, `Balance` — реляционная схема в PostgreSQL. `Balance` каждого юзера создаётся автоматически через сигнал `post_save` (`crypto/signals.py`).
...
* **Portfolio**: позиции пользователя — `user`, `symbol`, `amount`, `buy_price`, `bought_at`. Каждая покупка создаёт отдельную позицию (две покупки BTC по разным ценам = две записи).
* **Balance**: USD-баланс пользователя в формате 1-к-1 с `User`. Создаётся автоматически (нулевой) при регистрации через сигнал.
...
| `GET` | `/api/v1/portfolio/` | JWT | Позиции пользователя с текущей ценой и P&L (один SQL через annotate+Subquery) |
| `GET` | `/api/v1/portfolio/history/` | JWT | Динамика стоимости портфеля по снимкам (учитывает только позиции, купленные до даты снимка) |
| `POST` | `/api/v1/portfolio/buy/` | JWT | Покупка монеты по цене последнего снимка. Атомарно: списание + создание позиции |
| `POST` | `/api/v1/portfolio/positions/{position_id}/sell/` | JWT | Продажа монет из позиции. Атомарно: уменьшение/удаление + зачисление на баланс |
...
Все мульти-табличные операции (сохранение снимка, buy, sell) обёрнуты в `transaction.atomic` — гарантия «всё или ничего». Балансы и позиции защищены `select_for_update` от lost update при параллельных запросах. Побочные эффекты (cache invalidation, прогрев аналитики) выполняются через `transaction.on_commit`, чтобы кеш менялся только после фактического коммита БД.
...
| `/api/v1/portfolio/` | per-user low-level | 70 мин | при buy/sell (точечная) + при новом снимке (по паттерну для всех юзеров) |
| `/api/v1/portfolio/history/` | per-user low-level | 70 мин | то же самое |
